...
export default function Network() {
  ...
  const hasSentReq    = (uid) => {
    // We track this via checking other user's requests - approximate with local state
    return false; // Would need a separate API for "sent requests"
  };
  ...
  const handleConnect = async (uid) => {
    ...
    try {
      if (isConnected(uid)) {
        await API.delete(`/users/connections/${uid}`);
      } else {
        await API.post(`/users/${uid}/connect`);
      }
      await refreshUser();
  ...
  const handleRespond = async (fromId, action) => {
    ...
    try {
      await API.put(`/users/connections/${fromId}/respond`, { action });
      await refreshUser();
    } catch (e) { alert(e.response?.data?.message || 'Action failed'); }
    finally { setActionLoading(''); }
  };
    ...
    <div className="max-w-6xl mx-auto my-8 px-4">
              ...
              return (
                <div key={req.from} className="flex items-center gap-3 border border-gray-100 rounded-xl p-3">
                  ...
                  <div className="flex gap-2">
                    <button onClick={()=>handleRespond(req.from,'accept')} disabled={actionLoading===req.from} className="bg-blue-600 text-white px-3 py-1.5 rounded-lg text-xs font-semibold hover:bg-blue-700 transition">Accept</button>
                    <button onClick={()=>handleRespond(req.from,'reject')} disabled={actionLoading===req.from} className="border border-gray-200 text-gray-600 px-3 py-1.5 rounded-lg text-xs font-semibold hover:bg-gray-50 transition">Decline</button>
                  </div>
                </div>
              );
            })}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-5">
          ...
          <div className="flex gap-2">
            {['all','connected'].map(f=>(
              <button key={f} onClick={()=>setFilter(f)} className={`px-4 py-2 rounded-xl text-sm font-semibold capitalize transition ${filter===f?'bg-blue-600 text-white':'border border-gray-200 text-gray-600 hover:bg-gray-50'}`}>{f==='all'?'Everyone':'Connected'}</button>
            ))}
          </div>
        </div>
        ...
        <div className="relative mb-5">
          ...
          <input className="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" placeholder="Search by name, headline, or skill..." value={search} onChange={e=>setSearch(e.target.value)} />
        </div>
              ...
              return (
                <div key={u._id} className="border border-gray-100 rounded-2xl p-5 hover:border-blue-200 hover:shadow-md transition-all">
                  <div className="flex items-start gap-3 mb-3">
                    <img src={src} className="w-12 h-12 rounded-2xl object-cover border-2 border-gray-100" alt="" />
                    <div className="flex-1 min-w-0">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-lg font-medium flex-shrink-0 ${u.role==='employer'?'bg-purple-100 text-purple-600':u.role==='admin'?'bg-red-100 text-red-600':'bg-blue-100 text-blue-600'}`}>{u.role}</span>
                  </div>
                  {u.skills?.length > 0 && (
                  ...
                  {user && (
                    <div className="flex gap-2">
                      <button onClick={()=>handleConnect(u._id)} disabled={loading} className={`flex-1 py-2 rounded-xl text-xs font-semibold transition disabled:opacity-60 ${connected?'bg-gray-100 text-gray-700 hover:bg-red-50 hover:text-red-600':'bg-blue-600 text-white hover:bg-blue-700'}`}>
                      ...
                      <button onClick={()=>handleMessage(u._id)} disabled={actionLoading==='msg-'+u._id} className="flex-1 border border-gray-200 text-gray-600 py-2 rounded-xl text-xs font-semibold hover:bg-gray-50 transition disabled:opacity-60">
