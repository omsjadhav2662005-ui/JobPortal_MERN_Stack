      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex" style={{height:'calc(100vh - 160px)', minHeight:'500px'}}>
        ...
        <div className="w-72 border-r border-gray-100 flex flex-col flex-shrink-0">
          <div className="p-4 border-b border-gray-100">
            <h2 className="font-black text-lg text-gray-900">Messages {totalUnread>0&&<span className="ml-2 bg-red-500 text-white text-xs px-1.5 py-0.5 rounded-full">{totalUnread}</span>}</h2>
            <input className="mt-3 w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Search conversations..." value={searchConv} onChange={e=>setSearch(e.target.value)} />
          </div>
          <div className="flex-1 overflow-y-auto divide-y divide-gray-50">
            {sorted.length===0 ? (
              <div className="text-center py-12 px-4"><i className="fas fa-comment-dots text-gray-200 text-3xl mb-2 block"></i><p className="text-gray-400 text-sm">No conversations yet</p><p className="text-gray-300 text-xs mt-1">Go to Network to message someone</p></div>
            ) : sorted.map(conv=>{
              ...
              return (
                <div key={conv._id} onClick={()=>openConv(conv)} className={`flex items-center gap-3 px-4 py-3 cursor-pointer hover:bg-gray-50 transition ${selected?._id===conv._id?'bg-blue-50':''}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center"><p className={`text-sm truncate ${u>0?'font-bold':' font-semibold'}`}>{other?.name}</p><span className="text-xs text-gray-400 flex-shrink-0 ml-1">{last&&fmtDate(last.createdAt)}</span></div>
                    <p className={`text-xs truncate mt-0.5 ${u>0?'text-gray-700 font-medium':'text-gray-400'}`}>{last?`${isMe?'You: ':''}${last.text}`:'No messages yet'}</p>
                  </div>
                </div>
              );
            })}
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {!selected ? (
            ...
            <>
              {/* Header */}
              {(() => { const other=getOther(selected); const src=other?.profilePicture?imgUrl(other.profilePicture):getAvatar(other?.name); return (
                <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100 bg-white">
                  ...
                  <div><p className="font-bold text-sm">{other?.name}</p><p className="text-xs text-gray-400">{other?.headline||other?.email}</p></div>
                </div>
              ); })()}
                  ...
                  return (
                    <div key={i} className={`flex ${isMe?'justify-end':'justify-start'}`}>
                      <div className={`max-w-[70%] ${isMe?'items-end':'items-start'} flex flex-col`}>
                        ...
                        {(!selected.messages[i+1] || (selected.messages[i+1]?.from?._id||selected.messages[i+1]?.from)!==(msg.from?._id||msg.from)) && (
                          <span className={`text-[10px] mt-1 ${isMe?'text-gray-400':'text-gray-400'} mx-1`}>
              ...
              <form onSubmit={handleSend} className="flex gap-2 px-4 py-3 border-t border-gray-100 bg-white">
                <input type="text" value={text} onChange={e=>setText(e.target.value)} placeholder="Type a message..." className="flex-1 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-50" />
                <button type="submit" disabled={sending||!text.trim()} className="w-10 h-10 bg-blue-600 hover:bg-blue-700 text-white rounded-xl flex items-center justify-center transition disabled:opacity-50 flex-shrink-0">
